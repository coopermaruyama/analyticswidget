...
var reportMode = "visit";
var reportSpan = '3m';

var updateFreq = 1*60*60; // check for new data every hour
var homeURL = "http://vladalexa.com/apps/osx/analyticswidget/";
var analyticsSiteURL = "https://www.google.com/analytics/reporting/dashboard?id=";
var newVerURL = "http://vladalexa.com/apps/osx/analyticswidget/version.txt";
var downloadURL = "http://vladalexa.com/apps/osx/analyticswidget/AnalyticsWidget.wdgt.zip";
var timerInterval = 5*60*60; // check for new version update every 5 hours
...
function doPreloadSetup() {
	...
	setupPrefs();
	setupGliders();
	setupChartController(reportSpan);
	setupReportController(reportMode);
...
function setupPrefs() {
	if (window.widget) {
		KeyChainAccess.setAppName("AnalyticsWidget");

		// get the preference
		// Username
		var pref = widget.preferenceForKey(gkey("username"));
		if (pref != null) {
			username = pref;
		...
		// SiteID
		pref = widget.preferenceForKey(gkey("siteID"));
		if (pref != null) {
			siteID = pref;
		}
		
		// SiteName
		pref = widget.preferenceForKey(gkey("siteName"));
		if (pref != null) {
			siteName = pref;
		...
		// reportMode
		pref = widget.preferenceForKey(gkey("reportMode"));
		if (pref != null) {
			reportMode = pref;
		}

		// reportSpan
		pref = widget.preferenceForKey(gkey("reportSpan"));
		if (pref != null) {
			reportSpan = pref;
		}

		// SiteList
		pref = widget.preferenceForKey(gkey("siteList"));
		if (pref != null) {
...
function onremove() {
	...
	if (window.widget) {
		widget.setPreferenceForKey(null,gkey("username"));
		widget.setPreferenceForKey(null,gkey("siteID"));
		widget.setPreferenceForKey(null,gkey("siteName"));
		widget.setPreferenceForKey(null,gkey("siteList"));
		widget.setPreferenceForKey(null,gkey("reportMode"));
		widget.setPreferenceForKey(null,gkey("reportSpan"));
	}
}
...
//			emptyStatTable();
		...
		if (window.widget) {
			widget.setPreferenceForKey(siteID,gkey("siteID"));
			widget.setPreferenceForKey(siteName,gkey("siteName"));
			widget.setPreferenceForKey(encodeSiteList(siteList),gkey("siteList"));
			widget.setPreferenceForKey(reportMode,gkey("reportMode"));
			widget.setPreferenceForKey(reportSpan,gkey("reportSpan"));			
		}
	}
...
function getSiteList() {
	...
	if (window.widget)
		widget.system("./AnalyticsWidget.php \"" + encodeURIComponent(username) + "\" \"" + encodeURIComponent(password) + "\" sites ",getSiteListEH).onreadoutput = getSiteListOH;
...
function getSiteListOH(php_out) {
	...
	//siteList = JSON.parse(php_out);	
	siteList = php_out.evalJSON();
	...
	if (window.widget) {
		widget.setPreferenceForKey(username,gkey("username"));
//		widget.setPreferenceForKey(password,gkey("password"));
...
function getAnalyticsData() {
	...
	if (siteID && siteID > 0 && timeDiff(lastChecked, getCurrentTime()) > updateFreq ) {
		...
		if (window.widget)
			widget.system("./AnalyticsWidget.php \"" + encodeURIComponent(username) + "\" \"" + encodeURIComponent(password) + "\" stats " + siteID + " " + widget.identifier + " " + reportSpan,getSiteStatsEH).onreadoutput = getSiteStatsOH;
...
// Pass current time parmeter and expires to stop caching!?
	new Ajax.Request('data/analyticswidget.'+ widget.identifier +'.json', {
		method: 'get',
		parameters: {expires:getCurrentTime()+6, currenttime: getCurrentTime(), widgetid: widget.identifier, random: Math.random() },
...
function dayclick(event, td, tag) {
	...
	if (currentChartSelect != td) {
		...
		if (window.widget) {
			widget.setPreferenceForKey(tag, gkey("reportSpan"));
		}	
