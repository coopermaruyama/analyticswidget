...
function drawBigGraph(e, graphdata) {
	...
	$(e + '-narrative').innerHTML = graphdata.narrative;
...
function drawSparkline(e, data, numformat) {
	...
	spark_canvas.height = 30;
	spark_canvas.width = 120;
	...
	summary_span.innerHTML = data.summary.Item.SummaryValue + ' ' + data.summary.Message;
...
function drawTable(e, tabledata) {
	...
	tdhead.addClassName(reportMode + '-col1');
	...
	tabledata.data.each(function(itemhead) {
		...
		tddata.innerHTML = itemhead.Cell[1].Content.Value;
		tddata.title = itemhead.Cell[0].Content.Value;
