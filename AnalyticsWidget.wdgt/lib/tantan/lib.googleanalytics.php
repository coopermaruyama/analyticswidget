...
class tantan_GoogleAnalytics {
    ...
    function tantan_GoogleAnalytics() {
        ...
        if (function_exists('curl_init')) {   
            if (!class_exists('TanTanCurl')) require_once(dirname(__FILE__).'/lib/curl.php');     
    ...
    function login($user, $passwd) {
        ...
        */
        ...
        $hidden['GALX'] = $cookies['GALX']['value'];
        $hidden['GoogleAccountsLocale_session'] = $cookies['GoogleAccountsLocale_session']['value'];
        ...
        */
        ...
        $this->req->addPostData('submit', 'Sign in');
        $this->req->addPostData('GA3T', $hidden['GA3T']);
        $this->req->addPostData('GALX', $hidden['GALX']);
        $this->req->addCookie('GA3T', $hidden['GA3T']);
        $this->req->addCookie('GALX', $hidden['GALX']);
        $this->req->addCookie('GoogleAccountsLocale_session', $hidden['GoogleAccountsLocale_session']);
        ...
        */
        if (!ereg('LoginDoneHtml', $nextLink)) {
...
//??
        ...
        $nextLink = "http://www.google.com/analytics/home/?et=reset&amp;hl=en-US";
        if (!$nextLink) {
    ...
    function getReportData($profile, $start, $stop, $reportType='', $mode='XML') {
        ...
        switch ($reportType) {
            ...
            break;
			case 'trafsources':
				$url .= "&rpt=TrafficSourcesReport";
            break;
			case 'content':
				$url .= "&rpt=ContentReport";
		    break;
            case 'topcontent':
		...
		//DEBUG
		//file_put_contents ("/Users/valexa/Desktop/debug.txt" , $url."\n",FILE_APPEND);
